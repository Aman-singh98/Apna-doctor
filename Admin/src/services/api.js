...
/**
 * GET /admin/dashboard
 * → { success, stats: { totalDoctors, totalPatients, appointmentsToday, revenueThisMonth },
 *     revenueTrend: [{ month, revenue }],
 *     quickStats: { pendingVerifications, activeConsultations, openSupportTickets, refundRequests, newPatientsToday },
 *     recentAppointments: [{ id, patient, doctor, type, time, status }] }
 */
export const apiGetDashboard = () =>
	fetch(`${BASE}/admin/dashboard`, { headers: headers() }).then(handle);
 ...
 * GET /admin/payments
 * @param {{ status?: string, search?: string, page?: number, limit?: number }} params
 * → { success, total, page, pages, payments[] }
 */
export const apiGetPayments = (params = {}) => {
	const qs = new URLSearchParams(
		Object.fromEntries(Object.entries(params).filter(([, v]) => v !== '' && v != null))
	).toString();
	return fetch(`${BASE}/admin/payments${qs ? `?${qs}` : ''}`, { headers: headers() }).then(handle);
};

/** GET /admin/payments/stats → { success, stats: { total, paid, pending, refunded, failed, totalCollected } } */
export const apiGetPaymentStats = () =>
	fetch(`${BASE}/admin/payments/stats`, { headers: headers() }).then(handle);

/** GET /admin/payments/:id → { success, payment } */
export const apiGetPaymentById = (id) =>
 ...
 */
export const apiRefundPayment = (id, reason) =>
	fetch(`${BASE}/admin/payments/${id}/refund`, {
		method: 'PATCH',
		headers: headers(),
		body: JSON.stringify({ reason }),
	}).then(handle);
