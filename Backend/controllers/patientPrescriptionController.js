...
// See prescriptionController.js for why `email` is deliberately excluded.
...
//
// Lets the logged-in patient download their own prescription as a PDF
// slip. Same identity filter (linked patient id OR matching phone) as the
// two read handlers above, so this can never leak another patient's Rx.
exports.downloadMyPrescriptionPdf = async (req, res) => {
   try {
      const patient = await Patient.findById(req.user.id).select('phone gender dob bloodGroup');
      if (!patient) {
         return res.status(404).json({ message: 'Patient not found' });
      }
      ...
      // We already have this patient's own profile (gender/dob/bloodGroup)
      // from the lookup above — pass it straight through for the PDF's
      // patient-details line, no extra query needed.
      const prescriptionObj = prescription.toObject();
      prescriptionObj.patientProfile = patient.toObject();
