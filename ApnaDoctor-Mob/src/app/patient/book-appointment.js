...
import { bookAppointment, createPaymentOrder } from '../../services/patientAppointmentService';
...
export default function BookAppointmentScreen() {
   ...
   const [pendingOrder, setPendingOrder] = useState(null); // { orderId, amount, currency, keyId }
   const [pendingPayload, setPendingPayload] = useState(null); // the booking payload to send after payment verifies
   ...
   const confirmBooking = async () => {
      ...
      try {
         ...
         if (selectedPatient?.kind === 'family') {
            basePayload.familyMemberId = selectedPatient.id;
         }

         // Step 1: ask the backend to create a Razorpay order. The amount
         // is computed server-side from the doctor's category — never sent
         // from here (see createOrder in patientAppointmentController.js).
         const order = await createPaymentOrder(basePayload);

         setPendingPayload(basePayload);
         setPendingOrder(order);
         setCheckoutVisible(true);
      } catch (err) {
         Alert.alert('Error', err?.response?.data?.message || 'Could not start payment. Please try again.');
      } finally {
         setBookingLoading(false);
      }
   };

   // Step 2: fires only after Razorpay Checkout returns a signed success
   // response. The backend re-verifies that signature itself before ever
   // creating the Appointment — this call cannot book anything on its own.
   const handlePaymentSuccess = async ({ razorpay_order_id, razorpay_payment_id, razorpay_signature }) => {
      ...
      try {
         ...
         Alert.alert(
            ...
            err?.response?.data?.message || 'Your payment went through but we could not confirm the booking. Please contact support with your payment ID: ' + razorpay_payment_id
         );
      } finally {
         setBookingLoading(false);
   ...
   const handlePaymentError = (message) => {
      ...
      setPendingOrder(null);
      setPendingPayload(null);
      Alert.alert('Payment Failed', message || 'Something went wrong with the payment. Please try again.');
   };
   ...
   return (
      <SafeAreaView style={styles.safe}>
            ...
            {step === 3 && (
               <View>
                  ...
                  >
                     ...
                     <Text style={styles.confirmBtnTxt}>{bookingLoading ? 'Starting Payment...' : `Pay ₹${totalFee} & Confirm`}</Text>
                  </TouchableOpacity>
         ...
         <RazorpayCheckoutModal
            ...
            order={pendingOrder}
            name="ApnaDoctor"
            description={selectedDoc ? `${selectedMode.label} with Dr. ${selectedDoc.name}` : 'Consultation payment'}
            prefill={{
               name: myProfile?.name || selectedPatient?.name || '',
               email: myProfile?.email || '',
               contact: myProfile?.phone || '',
            }}
