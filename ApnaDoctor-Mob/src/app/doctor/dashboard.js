...
export default function DoctorDashboard() {
   ...
   // Today's patients / month count / today earnings for the stats row —
   // previously hardcoded even though getDashboardStats() was already being
   // fetched for `available`.
   const [dashStats, setDashStats] = useState({ todayPatients: null, monthCount: null, todayEarnings: null });
   ...
   useEffect(() => {
      ...
      (async () => {
         try {
            ...
            if (isMounted) {
               if (typeof stats?.available === 'boolean') {
      ...
      <SafeAreaView style={styles.safe}>
         ...
         <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.statsRow}>
               {[
                  { label: "Today's patients", value: dashStats.todayPatients != null ? String(dashStats.todayPatients) : '—', icon: 'person', color: TEAL, bg: '#E8F5F7' },
                  { label: 'This month', value: dashStats.monthCount != null ? String(dashStats.monthCount) : '—', icon: 'calendar', color: '#378ADD', bg: '#E6F1FB' },
                  { label: 'Today earnings', value: dashStats.todayEarnings != null ? `₹${dashStats.todayEarnings.toLocaleString('en-IN')}` : '—', icon: 'cash', color: GREEN, bg: '#E1F5EE' },
