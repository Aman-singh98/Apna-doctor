...
const api = axios.create({
   ...
   headers: {
      ...
      // Free-tier ngrok tunnels (EXPO_PUBLIC_API_URL pointing at an
      // *.ngrok-free.* URL) serve an HTML interstitial warning page to any
      // request that looks like it's coming from a browser, instead of
      // forwarding to the backend. This header tells ngrok to skip that
      // page and forward straight through. Harmless against a non-ngrok
      // backend (e.g. once you switch back to your LAN IP or a real
      // domain) — the header is simply ignored.
...
api.interceptors.request.use(
   async (config) => {
      ...
      // Let React Native's networking layer generate its own
      // 'multipart/form-data; boundary=...' header for FormData bodies
      // (file uploads). The instance-level default above sets
      // 'Content-Type: application/json' on every request, and RN only
      // auto-generates the multipart header (with the required boundary)
      // when no Content-Type is already present — otherwise it ships the
      // request as JSON with a FormData body attached, which the server's
      // multer middleware can't parse. That silently produces an empty
      // req.files on the backend (see doctorMeController.completeSignup),
      // which is why doctor-signup.js's file uploads were failing even
      // though the FormData itself was built correctly.
